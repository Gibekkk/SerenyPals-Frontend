...
class PetState extends ChangeNotifier {
  ...
  bool get isAngry => _consecutiveCriticalTicks >= 5; // Status marah tetap ada untuk logika lain (misal: berkedip)
  ...
  Future<void> _playBackgroundMusicAsset() async {
    ...
    try {
      ...
      print('Please check:');
      print('1. Is "assets/audio/background_music.mp3" the correct path in pubspec.yaml and in your project folder?');
...
class _VirtualPetPageState extends State<VirtualPetPage> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget petImage = Image.asset('assets/capybara_pet.png', width: 250, height: 250);
                ...
                Align(
                  ...
                  child: Padding(
                    ...
                    child: Container(
                      ...
                      child: IconButton(
                        icon: Icon(
                          petState.isMusicPlaying ? Icons.music_note : Icons.music_off,
                          color: Colors.black,
                        ),
                        onPressed: () {
                          Provider.of<PetState>(context, listen: false).toggleBackgroundMusic();
                        },
                      ),
                    ),
                  ),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildActionButton(context, Icons.fastfood, 'Makan', petState.hunger, petState.energy, () {
                  Provider.of<PetState>(context, listen: false).startEating();
                }),
                // Tombol "Main" - Warna berdasarkan energy (akan selalu normal di sini)
                _buildActionButton(context, Icons.directions_walk, 'Main', petState.hunger, petState.energy, () {
                  Provider.of<PetState>(context, listen: false).startPlaying();
                }),
                // Tombol "Tidur" - Warna berdasarkan energy
                _buildActionButton(context, Icons.nightlight_round, 'Tidur', petState.hunger, petState.energy, () {
                  Provider.of<PetState>(context, listen: false).startSleeping();
                }),
                // Tombol "Toko" - Tidak berubah warna
                _buildActionButton(context, Icons.store, 'Toko', 100, 100, () { // Pass nilai default agar tidak berubah
                  // Handle Toko action
                }),
              ],
            ),
  ...
  Widget _buildActionButton(
      BuildContext context, IconData icon, String label, double currentHunger, double currentEnergy, VoidCallback onPressed) {
      ...
      children: [
        InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: buttonBackgroundColor, // Warna latar belakang tombol berubah
              shape: BoxShape.circle,
            ),
            child: Icon(
              icon,
